...
module des_spru(row, dot=false, deepdish=false, n=spru_n, width=1, radius=spru_radius) {
    echo ("Row", row, "width", width);
    union() {
        for (i = [0 : n - 1]){
            translate([i * spacing * width, 0, 0])
            mirror([0,0,0])
            des_keycap(row=row, width=width, dot=dot, deepdish=deepdish);
        }
...
module des_keycap(row, width=1, dot=false, deepdish=false) {
    ...
    else if (row == 3 && width == 1 && !deepdish) {des_standard(1, dot=dot);}
...
module des_standard(keyID, dot=false) {
    echo("keyID", keyID)
    keycap(
